...
static pkt_desc_t *NewPkt(struct ether_header *peth, struct ip *pip, void *ptcp,
                          void *plast) {
  ip_packet *ppkt;
  ...
  while ((num_ip_packets != old_new_ip_packets) &&
         (pkt_arr[num_ip_packets] != NULL) && (steps < LIST_SEARCH_DEPT)) {
    steps++;
...
static flow_hash_t *FindFlowHash(struct ip *pip, void *ptcp, void *plast,
                                 int *pdir) {
  /* Start to check */
  ...
  for (flow_hash_ptr = flow_hash_table[hash_index]; flow_hash_ptr;
...
void check_timeout_lazy() {
  ...
  for (ix = entry_index; ix < entry_index + FLOW_HASH_TABLE_GC_SIZE; ix++) {
    ...
    for (flow_hash_ptr = flow_hash_head_ptr; flow_hash_ptr;
         flow_hash_ptr = flow_hash_ptr->next) {
      if (flow_hash_ptr->lazy_pending == TRUE) {
        if (tv_sub_2(current_time, flow_hash_ptr->last_pkt_time) >=
...
void check_timeout_periodic() {
  ...
  for (ix = 0; ix < PKT_BUF_GC_SPLIT_SIZE; ix++) {
    ...
    //        inet_ntop(AF_INET, &tmp_ppkt->addr_pair.a_address.un.ip4,
    //        ip_src_addr_print_buffer, INET_ADDRSTRLEN),
    //        ntohs(tmp_ppkt->addr_pair.a_port),
    //        inet_ntop(AF_INET, &tmp_ppkt->addr_pair.b_address.un.ip4,
    ...
    if (tmp_ppkt_desc->is_replied) {
      ...
      if (elapsed_time >= PKT_TIMEOUT) {
        ...
        if (SendPktCollector(tmp_ppkt_desc->ppkt->raw_pkt,
                             tmp_ppkt_desc->pkt_len) == -1) {
...
static flow_hash_t *CreateFlowHash(struct ether_header *peth, struct ip *pip,
                                   void *ptcp, void *plast) {
...
int pkt_handle(struct ether_header *peth, struct ip *pip, void *ptcp,
  ...
  if (flow_hash_ptr != NULL) {
    ...
    /* Reversed direction of this packet, probably a response */
      ...
      //     //     inet_ntop(AF_INET,
      //     &flow_hash_ptr->addr_pair.a_address.un.ip4,
      //     ip_src_addr_print_buffer, INET_ADDRSTRLEN),
...
void trace_init(void) {
  ...
  memset(&if_idx_collector, 0, sizeof(struct ifreq));
  strncpy(if_idx_collector.ifr_name, ifName_collector, IFNAMSIZ - 1);
  if (ioctl(sockfd_collector, SIOCGIFINDEX, &if_idx_collector) < 0)
    perror("SIOCGIFINDEX");
  socket_address_collector.sll_family = AF_PACKET;
  socket_address_collector.sll_ifindex = if_idx_collector.ifr_ifindex;
  ...
  /* Get the index of the interface to send on */
  memset(&if_idx_switch, 0, sizeof(struct ifreq));
  strncpy(if_idx_switch.ifr_name, ifName_switch, IFNAMSIZ - 1);
  if (ioctl(sockfd_switch, SIOCGIFINDEX, &if_idx_switch) < 0)
    perror("SIOCGIFINDEX");
